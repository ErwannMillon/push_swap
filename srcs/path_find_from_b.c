...
/*   Updated: 2022/05/21 10:34:04 by gmillon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

t_arr	reinsert_before_target(t_node *num, t_node *a, t_node *b, t_arr sortedarr)
{
	const int	num_dist_top = ft_get_list_index(num->num, b);
	const int	num_dist_bottom = dist_to_bottom(num->num, b);
	const int	target_index = ft_get_list_index(num->next_in_sorted, a);
