...
/*   Updated: 2022/09/18 12:18:36 by gmillon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
									...
									t_node **b, t_arr path)
{
	...
	i = 0;
	if (dist_to_top(target->num, *a) - num_dist_top \
		> dist_to_bottom(target->num, *a) + 1)
	{
		path = move_to_top(num->num, b, RB);
		path = extend_path(path, call_n_times(&rr, \
			RRA, dist_to_bottom(target->num, *a) + 1, a));
	...
	if (get_list_index(target->num, *a) != 0)
		path = extend_path(path, call_n_times(&r, \
			RA, dist_to_top(target->num, *a), a));
	return (extend_path(path, push_path(b, a, PA)));
}
										...
										t_node **b, t_arr path)
{
	...
	{
		path = extend_path(path, call_n_times(&rr, RRB, \
			num_dist_bottom + 1, b));
		path = extend_path(path, call_n_times(&r, RA, \
			dist_to_top(target->num, *a), a));
	}
	else
	{
		while (i < num_dist_top)
		{
			path = align_arrs(target, a, b, path);
			i++;
		}
		if (get_list_index(target->num, *a) != 0)
			path = extend_path(path, call_n_times(&rr, RRA, \
				dist_to_bottom(target->num, *a) + 1, a));
	}
	return (extend_path(path, push_path(b, a, PA)));
}
