...
t_arr	top_insert_before_closest(t_node *num, t_node **a, t_node **b, t_arr path)
{
	...
	i = 0;
	if (dist_top(target->num, *a) - num_dist_top > dist_bottom(target->num, *a) + 1)
	{
		path = move_to_top(num->num, b, RB);
		path = extend_path(path, call_n_times(&rr, RRA, dist_bottom(target->num, *a) + 1, a));
	...
	if (ft_get_list_index(target->num, *a) != 0)
		path = extend_path(path, call_n_times(&r, RA, dist_top(target->num, *a), a));
	return (extend_path(path, push_path(b, a, PA)));
}
