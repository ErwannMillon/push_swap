...
def move_unsorted_to_b(a, b):
	...
	# 		if first_half[0] == num or min(dist_bottom(a, num), dist_top(a, num)) < min_len:
	...
	# 		if  second_half[0] == num or min(dist_bottom(a, num), dist_top(a, num)) < min_len:
	# 			min_len = min(dist_bottom(a, num) + 1, dist_top(a, num))
	# 			num_to_push = num
	# 	path.extend(move_to_top(num_to_push, a))
	# 	p(a, b)
	...
	if (len(a) > 200):
		...
		while (unsorted):
			...
			unsorted.pop(unsorted.index(num_to_push))
			print("NUM TO PUSH", num_to_push)
	print("after move unsorted to B")
	print_stacks(a, b)
		# print(path)
	# print(unsorted)
		# print()
...
fail = [3, 6, 7, 9, 10, 2, 8, 1, 0, 5, 4]
checkcopy = copy.deepcopy(fail)
path = main(fail)[0]
print()
for item in path:
	print(item)
